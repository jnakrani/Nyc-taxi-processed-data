...
RIDE_INFO_PATH = (
    "nyc-taxi-orig-cleaned-split-parquet-per-year-multiple-files/ride-info/"
)
RIDE_FARE_PATH = (
    "nyc-taxi-orig-cleaned-split-parquet-per-year-multiple-files/ride-fare/"
)
...
def start_timer(stage_name):
    ...
    logger.info(
        f"Starting stage: {stage_name} at {timing_stats['stages'][stage_name]['start_time']}"
    )
...
def end_timer(stage_name):
    ...
    if stage_name in timing_stats["stages"]:
        ...
        duration = (
            timing_stats["stages"][stage_name]["end_time"]
            - timing_stats["stages"][stage_name]["start_time"]
        )
...
def save_timing_report():
    ...
    json_compatible_stats = {
        "start_time": (
            timing_stats["start_time"].isoformat()
            if timing_stats["start_time"]
            else None
        ),
        "end_time": (
            timing_stats["end_time"].isoformat() if timing_stats["end_time"] else None
        ),
        "total_duration": (
            str(timing_stats["total_duration"])
            if timing_stats["total_duration"]
            else None
        ),
        "stages": {},
    }
    ...
    with open(summary_file, "w") as f:
        ...
        f.write(
            f"Total Duration: {format_duration(timing_stats['total_duration'])}\n\n"
        )
...
def save_checkpoint(
    processed_files, data_type, total_files, total_processed, has_data=False
):
    ...
    checkpoint = {
        ...
        "has_data": has_data,
        "elapsed_time": (
            str(datetime.now() - timing_stats["start_time"])
            if timing_stats["start_time"]
            else None
        ),
    }
...
def read_parquet_with_dask(s3_client, bucket, file_key):
    ...
    try:
        ...
        ddf = dd.read_parquet(
            s3_path,
            storage_options={"use_ssl": True, "anon": False},  # Use AWS credentials
            engine="pyarrow",
        ...
        if "pickup_datetime" in ddf.columns and "dropoff_datetime" in ddf.columns:
            ddf["trip_duration"] = (
                ddf["dropoff_datetime"] - ddf["pickup_datetime"]
            ).dt.total_seconds() / 60
    ...
    try:
        ...
        if ddf is not None:
            ...
            temp_filename = os.path.join(
                TEMP_DIR, f"{data_type}_{os.path.basename(file_key)}.parquet"
            )

            # Use Dask's to_parquet - this triggers computation but with better memory management
            ddf.to_parquet(
                temp_filename, compression="snappy", write_index=False, engine="pyarrow"
            )

            # Log success
            logger.info(
                f"Successfully processed {data_type} file: {file_key} ({rows} rows)"
            )
            log_memory_usage()
...
def process_batch(batch_files, s3_client, bucket, data_type):
    ...
    if free_gb is not None and free_gb < 5.0:
        ...
        logger.info(
            f"Reducing worker count due to low disk space: {effective_workers} workers"
        )
    else:
    ...
    with ThreadPoolExecutor(max_workers=effective_workers) as executor:
        future_to_file = {
            executor.submit(process_file_with_new_client, file_key): file_key
    ...
    logger.info(
        f"Batch completed: {len(processed_files)}/{len(batch_files)} files processed in {elapsed_time:.2f} seconds"
    )
    logger.info(
        f"Performance: {files_per_second:.2f} files/sec, {rows_per_second:.2f} rows/sec"
    )
                ...
                try:
                    if temp_file and os.path.exists(temp_file):
                        ...
                        if processed_count % 5 == 0:
                            logger.info(
                                f"Processed {processed_count}/{len(temp_files)} temporary files"
                            )
                except Exception as e:
...
def load_data_from_s3():
    ...
    s3 = session.client(
        "s3", config=Config(max_pool_connections=50, retries={"max_attempts": 10})
    )
    ...
    logger.info(
        f"Processing {ride_info_count} ride info files and {ride_fare_count} ride fare files"
    )
    ...
    if data_type == "ride_info" or data_type == "both":
        ...
        for i in range(0, len(unprocessed_ride_info), BATCH_SIZE):
            ...
            logger.info(
                f"Processing batch {current_batch}/{total_batches} of ride info files"
            )
            logger.info(f"Remaining batches after this one: {remaining_batches}")
            log_memory_usage()

            # Process batch
            temp_files, new_processed, batch_rows = process_batch(
                batch, s3, S3_BUCKET_PREFIX, "ride_info"
            )
            ...
            save_checkpoint(
                list(processed_files), "ride_info", total_files, total_processed
            )
            ...
            logger.info(
                f"  Files: {total_processed}/{total_files} ({percent_complete:.2f}%)"
            )
            ...
            expected_remaining_time = (
                (time.time() - start_time) / total_processed * remaining_files
                if total_processed > 0
                else 0
            )
            logger.info(
                f"  Estimated remaining time: {expected_remaining_time:.2f} seconds ({expected_remaining_time/60:.2f} minutes)"
            )
    ...
    for i in range(0, len(unprocessed_ride_fare), BATCH_SIZE):
        ...
        logger.info(
            f"Processing batch {current_batch}/{total_batches} of ride fare files"
        )
        logger.info(f"Remaining batches after this one: {remaining_batches}")
        log_memory_usage()
        ...
        # Process batch
        temp_files, new_processed, batch_rows = process_batch(
            batch, s3, S3_BUCKET_PREFIX, "ride_fare"
        )
    ...
    logger.info(
        f"Total ride info rows: {total_ride_info_rows}, Total ride fare rows: {total_ride_fare_rows}"
    )
    ...
    logger.info(
        f"Data loading completed in {total_time:.2f} seconds ({total_time/60:.2f} minutes)"
    )
    logger.info(f"Final dataset shape: {merged_df.shape}")

    # Save final checkpoint with data status and timing info
    save_checkpoint(
        list(processed_files), "both", total_files, total_processed, has_data=True
    )
...
def explore_data(df):
    ...
    if (
        "trip_duration" not in df.columns
        and "pickup_at" in df.columns
        and "dropoff_at" in df.columns
    ):
    ...
    plt.figure(figsize=(12, 6))
    sns.scatterplot(
        data=df.sample(n=10000, random_state=RANDOM_STATE),
        x="trip_distance",
        y="fare_amount",
        alpha=0.5,
    )
...
def main():
    ...
    try:
        ...
        timing_stats["total_duration"] = (
            timing_stats["end_time"] - timing_stats["start_time"]
        )
...
def check_disk_space(directory):
    ...
    try:
        ...
        logger.info(
            f"Disk space check - Free: {free_gb:.2f} GB, Used: {disk.percent:.1f}%"
        )

        # If less than 2GB free or >90% used, clean up temporary files
        if free_gb < 2.0 or disk.percent > 90:
            logger.warning(
                f"Low disk space detected! Free: {free_gb:.2f} GB, Used: {disk.percent:.1f}%"
            )
            ...
            if os.path.exists(TEMP_DIR):
                ...
                files_to_remove = (
                    len(temp_files) // 2 if disk.percent > 95 else len(temp_files) // 4
                )
                for filepath, _ in temp_files[:files_to_remove]:
                    try:
                        ...
                        logger.error(
                            f"Failed to remove temporary file {filepath}: {str(e)}"
                        )
            ...
            logger.info(
                f"After cleanup - Free: {free_gb:.2f} GB, Used: {disk.percent:.1f}%"
            )
...
def setup_dask_client():
    ...
    logger.info(
        f"Setting up Dask with {n_workers} workers, {threads_per_worker} threads per worker, {memory_limit} RAM per worker"
    )
...
def process_with_full_dask_pipeline():
    ...
    try:
        ...
        if TESTING_MODE:
            logger.info(
                f"TESTING MODE ENABLED: Processing only {TEST_FILE_COUNT} files from each dataset"
            )
        ...
        logger.info(
            f"Processing {len(ride_info_paths)} ride info files and {len(ride_fare_paths)} ride fare files"
        )
        ...
        if "pickup_datetime" in ride_info_ddf.columns:
            ...
            ride_info_ddf["pickup_dayofweek"] = ride_info_ddf[
                "pickup_datetime"
            ].dt.dayofweek

        if (
            "pickup_datetime" in ride_info_ddf.columns
            and "dropoff_datetime" in ride_info_ddf.columns
        ):
            ride_info_ddf["trip_duration"] = (
                ride_info_ddf["dropoff_datetime"] - ride_info_ddf["pickup_datetime"]
            ).dt.total_seconds() / 60
        ...
        train_ddf = merged_ddf.sample(
            frac=TRAIN_TEST_SPLIT_RATIO, random_state=RANDOM_STATE
        )
        ...
        val_ddf = merged_ddf.sample(
            frac=1 - TRAIN_TEST_SPLIT_RATIO, random_state=RANDOM_STATE + 1
        )
        ...
        logger.info(
            "Please provide the number of rows and columns for your training dataset:"
        )
        ...
        logger.info(
            "Please provide the number of rows and columns for your validation dataset:"
        )
...
if __name__ == "__main__":
    try:
        ...
        logger.info(
            f"Train/Test Split Ratio: {TRAIN_TEST_SPLIT_RATIO:.2f}/{1-TRAIN_TEST_SPLIT_RATIO:.2f}"
        )
